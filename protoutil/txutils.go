...
import (
	...
	"github.com/zhigui-projects/fabric-connector/protoutil/identity"
)

// GetPayloads gets the underlying payload objects in a TransactionAction
func GetPayloads(txActions *peer.TransactionAction) (*peer.ChaincodeInvocationSpec, *peer.ChaincodeActionPayload, *peer.ChaincodeAction, error) {
	...
	cpp, err := UnmarshalChaincodeProposalPayload(ccPayload.ChaincodeProposalPayload)
	if err != nil {
		return nil, nil, nil, err
	}

	cis, err := UnmarshalChaincodeInvocationSpec(cpp.Input)
	if err != nil {
		return nil, nil, nil, err
	}

	if ccPayload.Action == nil || ccPayload.Action.ProposalResponsePayload == nil {
		return cis, nil, nil, errors.New("no payload in ChaincodeActionPayload")
	}
	...
	if pRespPayload.Extension == nil {
		return cis, nil, nil, errors.New("response payload is missing extension")
	}

	respPayload, err := UnmarshalChaincodeAction(pRespPayload.Extension)
	if err != nil {
		return cis, ccPayload, nil, err
	}
	return cis, ccPayload, respPayload, nil
}
