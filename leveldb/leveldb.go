...
func CreateDB() *DB {
	dbPath := path.Join(GetCurrentPath(), "dbdata")
	log.Print("db path: ", dbPath)
...
func (db *DB) Close() {
	...
	if err := db.db.Close(); err != nil {
		log.Panic("Error closing leveldb", "error", err)
	}
	db.dbState = closed
}
...
func (f *FileLock) Unlock() {
	...
	if err := f.db.Close(); err != nil {
		log.Panic("unable to release the lock on file", "filePath", f.filePath, "error", err)
		return
	}
	f.db = nil
}
